...
Next, we perform post-hoc power calculations for transparency and to better understand the results by considering the limitations of our cohort. 

Finally, we explore spectrum bias within our cohort. Discussions of the inherent biases of autopsy cohort are found in the main article.

First though, we should read the data output from S2 Analysis.
...
ref1 <- dat %>% filter(complete.cases(final_ret)) %>% select(2:3)
...
Then we produce the comparative table for the included and missing cohorts (S4).
...
## 3 Post-hoc power calculations
Because our sample size is small for the sub-analysis (n=60) there is a high likelihood that this analysis is underpowered to detect a difference between the definitions of malarial retinopathy if a difference exists. Because `testCompareR` uses different methods depending on the sample size and disease prevalence, we cannot easily use asymptotic methods to calculate power. Instead, we can employ simulation to approximate our power and the sample size required to achieve 80% power to detect a difference. This will be useful for future research.

```{r message=FALSE, warning=FALSE}
#| code-fold: true
#| code-summary: "Expand to see the code used to produce S5 Figure"
...
sim_model <- function(n) {
  results <- replicate(n = 1000, expr = {
    ...
    testCompareR::compareR(test1 = boot_sample$ophth_ret,
                           test2 = boot_sample$recoded,
                           gold = boot_sample$true_cm)$acc$spec.p.adj < 0.05})
  
  mean(unlist(results), na.rm = TRUE)
  
}

sample_size <- seq.int(50, 200, 10)
power <- sapply(sample_size, sim_model)
power <- data.frame(sample_size, power)

power <- power %>%
  mutate(method = ifelse(sample_size <= 100, "exact", "wald"))

lm_exact <- lm(power ~ sample_size, data = subset(power, method == "exact"))
lm_wald  <- lm(power ~ poly(sample_size, 2, raw = TRUE), 
               data = subset(power, method == "wald"))

pred_power <- coef(lm_exact)[1] + 60*coef(lm_exact)[2]
...
pred_n <- ifelse(n1 >= 50 & n1 <= 200, n1, n2)

power$pred_quad <- NA
power$pred_exp <- NA

power$pred_quad[power$method == "exact"] <- predict(lm_exact)
power$pred_quad[power$method == "wald"]  <- predict(lm_wald)

g1 <- ggplot(power, aes(x = sample_size, y = power, color = method)) +
  geom_point(size = 2) +
  geom_line(aes(y = pred_quad), linetype = "dashed") +
  geom_segment(aes(x = 50, xend = pred_n, y = 0.80, yend = 0.80), 
               linetype = "dotted", colour = "blue") +
  geom_segment(aes(x = pred_n, xend = pred_n, y = 0.2, yend = 0.80), 
               linetype = "dotted", colour = "blue") +
  geom_segment(aes(x = 50, xend = 60, y = pred_power, yend = pred_power), 
               linetype = "dotted", colour = "purple") +
  geom_segment(aes(x = 60, xend = 60, y = 0.2, yend = pred_power), 
               linetype = "dotted", colour = "purple") +
  geom_text(aes(x = 160, y = 0.75, 
                label = paste0("n = ", round(pred_n, 0))), 
            colour = "black", hjust = -0.1) +
  geom_text(aes(x = 63, y = pred_power, 
                label = paste0("Power = ", round(pred_power*100, 1), "%")), 
            colour = "black", hjust = -0.1) +
...
filename <- "S5_Figure.pdf"
ggsave(filename, plot = g1, width = 8, height = 6)

g1
```

This shows that the sample size limitation inherent in our retrospective analysis left us underpowered and may explain in part why we are only seeing a trend towards significance. To be adequately powered to detect a difference in specificity of 12% (observed in this cohort) we would need to recruit 157 patients. 

## 4 Exploring spectrum bias
...
Because construction of the model matrix requires complete data, we used multiple imputation by chained equations to produce 5 data sets for LASSO logistic regression and evaluated how frequently predictors were retained following LASSO penalisation and the mean effect size. We ran the LASSO logistic regression using the original cohort (n=84).  
