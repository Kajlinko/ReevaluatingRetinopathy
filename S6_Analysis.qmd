...
dual <- read_xlsx(here("../data/ImageConcordanceData.xlsx"))
...
rtd <- read.csv(here("../data/RetinopathyConcordanceData.csv"))
...
# make the data match the photo grading data structure
rtd <- rtd %>% transmute(
  mp_number = mp_number,
  ahaem_r = case_when(
    haem_r == 7 ~ 4,
  ...
  acfa_l = case_when(
    fov_wh_l < 0 ~ NA_real_,
    TRUE ~ fov_wh_l),
  awhitcmac_r = case_when(
    mac_wh_r == 3 ~ 2,
    mac_wh_r == 4 ~ 3,
    mac_wh_r < 0 ~ NA_real_,
    TRUE ~ mac_wh_r),
  awhitcmac_l = case_when(
    mac_wh_l == 3 ~ 2,
    mac_wh_l == 4 ~ 3,
    mac_wh_l < 0 ~ NA_real_,
    TRUE ~ mac_wh_l),
  apw_r = if_else(per_wh_r %in% 1:12, 1,
                  if_else(per_wh_r == 0 & 
                         (wh_qu_seen_r == 3 | wh_qu_seen_r == 4), 0, NA_real_)),
  apw_l = if_else(per_wh_l %in% 1:12, 1,
                  if_else(per_wh_l == 0 & 
                         (wh_qu_seen_l == 3 | wh_qu_seen_l == 4), 0, NA_real_)),
  avc_r = if_else(quad_vc_r %in% 1:4, 1, 
                  if_else(quad_vc_r == 0 & 
                         (vc_qu_seen_r == 3 | vc_qu_seen_r == 4), 0, NA_real_)),
  avc_l = if_else(quad_vc_l %in% 1:4, 1,
                  if_else(quad_vc_l == 0 & 
...
rtd <- rtd %>%
  ...
  mutate(mp_number = paste0(mp_number, toupper(side))) %>%
  select(-side)
...
for (i in seq_along(features)) {
  ...
  direction <- if (adj_p < 0.05) "This is statistically significant." else "This is not statistically significant."
