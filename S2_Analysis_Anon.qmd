...
```{r message=FALSE, warning=FALSE}
# import data
tab <- read_xlsx(here("../AnonData/OriginalAutopsyData.xlsx")) %>%
  remove_empty()

# select and rename essential columns
tab <- tab %>% select(mp_number, CLASS, Ret)
...
```{r message=FALSE, warning=FALSE}
## ONLY USED WITH COMPLETE DATA - NOT ANONYMISED DATA
...
tab <- tab %>%
  mutate(
    ...
    class = if_else(mp_number == "MPa083", "7", as.character(class)),
...
chart <- read_xlsx(here("../AnonData/ChartReviewData.xlsx")) %>%
  remove_empty()
...
rtd <- read.csv(here("../AnonData/RetinopathyData.csv")) %>%
  remove_empty()
...
tab <- tab %>%
  mutate(final_ret = case_when(
    mp_number %in% c("MPa110", "MPa100", "MPa072") ~ NA_real_,
    mp_number %in% c("MPa020", "MPa025") ~ 1,
    mp_number %in% c("MPa031", "MPa030") ~ 0,
...
comments <- read_xlsx(here("../AnonData/ChartReviewData.xlsx"), sheet = 2)
#comments <- comments %>%
#  mutate(
...
``` 
***S2 Table 1. Consensus review of mismatched cases.** Review by two reviewers to establish the final retinal status in cases where the records and the chart review were mismatched. *
...
demo[,3:18] <- lapply(demo[,3:18], function(x) {
  x <- as.numeric(x)
  x <- ifelse(x < 0, NA, x)
  return(x)
})
...
***S2 Table 2. Demographics summarised by presence of cerebral parasite sequestration and presence of malarial retinopathy.** \* indicates number of participants meeting specified condition (percentage). All other values indicate the median (range). BCS – Blantyre Coma Score; BP – blood pressure; bpm – beats per minute; CSF – cerebrospinal fluid; rpm – respirations per minute. Corresponds to Table 1 in the manuscript.*
...
*The data output above contributes to Table 2 and Table 3 of the manuscript.*
...
* MPa079 had a single haemorrhage and macular oedema in 1 eye, diagnosis
* MPa034 had a single spot of retinal whitening in 1 eye, diagnosis 
* MPa042 had 1-5 haemorrhages only in 1 eye, diagnosis clinical severe malaria without coma
* MPa025 had peripheral whitening only in both eyes, diagnosis infectious encephalopathy (non-malaria)
* MPa108 had 1-5 haemorrhages only in 1 eye, diagnosis non-infectious encephalopathy
* MPa002 had haemorrhages only, many in right eye, diagnosis clinical CM without sufficient sequestration for diagnosis
* MPa018 had peripheral whitening only in both eyes, diagnosis clinical CM without sufficient sequestration for diagnosis
* MPa065 had MR with all signs, diagnosis CM without sufficient sequestration for diagnosis
* MPa017 had 1-5 haemorrhages only in 1 eye, diagnosis clinical CM without sufficient sequestration for diagnosis
* MPa092 had 1-5 haemorrhages in 1 eye and 6-20 haemorrhages in the other eye, diagnosis non-infectious encephalopathy
